...
| 大模型辅助归因 | 大模型提出 6 条假设，数据校验后 3 条成立、2 条部分成立、1 条不成立；定位主因为定价未跟上风险变化 |
...
- 三层策略：分数 ≥ 533 直接通过（71.5%），519–533 加验后放款（20.0%），< 519 拒绝（8.5%）。两个阈值用网格搜索求解：收益是分数阈值的阶梯函数，没有可用的梯度，而 200 × 200 的组合全部枚举能保证全局最优
...
### 4. 大模型辅助风控运营
- **归因**：大模型针对"2015 年坏账率为什么上升"提出 6 条假设，代码把坏账率变化拆成结构效应与组内效应逐条校验，并做人工复核（`output/attribution_human_review.md`）。结论：客群并未变差，主因是 A–F 各评级利率均下调而坏账率上升，定价没有跟上风险
